...
- **Command palette (`⌘K`)** — one launcher to fuzzy-search every tool *and* run any action (updates, security, disk, stars, themes, describe, rescan…) without touching the mouse.
...
catalog is built to run mouse-free — the search field is focused on launch, so you can start typing the moment it opens.

| Shortcut | Action |
| --- | --- |
| <kbd>⌘</kbd> <kbd>K</kbd> | Open / close the command palette |
| <kbd>↑</kbd> <kbd>↓</kbd> | Move through the list (or palette results) |
| <kbd>Home</kbd> / <kbd>End</kbd> | Jump to the first / last tool |
| <kbd>Enter</kbd> / <kbd>Space</kbd> | Open the highlighted tool, or expand / collapse a group |
| <kbd>F</kbd> | Star / unstar the highlighted tool |
| <kbd>Esc</kbd> | Close the detail panel, the open sheet, or the palette |

From the **command palette** (<kbd>⌘</kbd> <kbd>K</kbd>) you can fuzzy-search across every installed tool *and* jump to any action by name — **Check for updates, Disk usage, Security scan, GitHub stars, Export setup, Health check, Themes, Settings, Describe items, Rescan library,** and **Toggle light / dark**. Arrow to a result and press <kbd>Enter</kbd>.
