...
const TOOLS: &[(&str, &str, &str)] = &[
    ("brew",  "Homebrew formulae & casks (+ update checks)", "https://brew.sh"),
    ("mas",   "Mac App Store apps (+ updates)", "brew install mas"),
    ("npm",   "global npm packages (+ updates, vuln scan)", "Node.js — nvm or brew install node"),
    ("cargo", "Rust cargo-install tools (+ vuln scan)", "https://rustup.rs"),
...
pub fn check_deps() -> Vec<Dep> {
    ...
    TOOLS.iter().map(|(name, powers, install)| Dep {
        name: (*name).to_string(),
...
mod tests {
    ...
    fn check_deps_lists_all_eight_tools() {
        ...
        assert!(d.iter().all(|x| !x.powers.is_empty() && !x.install.is_empty()));
    }
}
