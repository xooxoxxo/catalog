...
pub fn decide_removable(
    ...
    match source {
        "brew" | "cask" if unneeded_dep => {
            (true, Some("installed as a dependency; nothing needs it now".into()))
        }
        "app" | "mas" => {
...
pub fn analyze<F: Fn(usize, usize)>(items: &[Item], progress: F) -> Vec<DiskInfo> {
    ...
    for (i, item) in items.iter().enumerate() {
        ...
        let unneeded_dep =
            item.source == "brew" && item.installed_on_request == Some(false) && leaves.contains(&item.name);
...
mod tests {
    ...
    fn parse_du_kb_reads_first_field_to_bytes() {
        assert_eq!(parse_du_kb("4200\t/opt/homebrew/Cellar/llvm"), Some(4200 * 1024));
    ...
    fn decide_removable_app_unused_and_big() {
        ...
        let (r, why) = decide_removable("app", Some(400 * 1024 * 1024), Some((2024, 9)), (2026, 6), false);
        assert!(r);
    ...
    fn decide_removable_app_recent_or_small_not_flagged() {
        // recent
        assert!(!decide_removable("app", Some(400 * 1024 * 1024), Some((2026, 5)), (2026, 6), false).0);
        // old but small
        assert!(!decide_removable("app", Some(10 * 1024 * 1024), Some((2020, 1)), (2026, 6), false).0);
        // no usage date
