...
pub fn store_path() -> PathBuf {
    crate::config::config_base().join("catalog").join("enrichment.toml")
}
...
pub fn merge(item: &crate::item::Item, enr: Option<&Enrichment>) -> EnrichedItem {
    ...
    let display_name = if e.alias.trim().is_empty() { item.name.clone() } else { e.alias.clone() };
    let description = if e.description.trim().is_empty() { item.raw_desc.clone() } else { Some(e.description.clone()) };
    EnrichedItem {
        id: item.id.clone(),
...
mod store_tests {
    ...
    fn save_then_load_roundtrips() {
        ...
        s.set("brew:eza".into(), Enrichment { favorite: true, tags: vec!["cli".into()], ..Default::default() });
        save_to(&p, &s).unwrap();
        let loaded = load_from(&p);
        assert_eq!(loaded.get("brew:eza").unwrap().favorite, true);
        assert_eq!(loaded.get("brew:eza").unwrap().tags, vec!["cli".to_string()]);
    ...
    fn set_default_removes_entry() {
        let mut s = EnrichmentStore::default();
        s.set("x".into(), Enrichment { favorite: true, ..Default::default() });
...
mod merge_tests {
    ...
    fn item() -> Item {
        Item { id: "brew:eza".into(), name: "eza".into(), source: "brew".into(), source_detail: None,
            version: None, exec_path: None, homepage: None, raw_desc: Some("modern ls".into()), installed_on_request: None }
    }
    ...
    fn no_enrichment_uses_detected() {
        ...
        assert_eq!(m.favorite, false);
        assert_eq!(m.has_enrichment, false);
    }

    #[test]
    fn alias_overrides_name_and_desc_overrides_raw() {
        let e = Enrichment { alias: "eza (ls)".into(), description: "my note".into(), favorite: true, hidden: true, ..Default::default() };
    ...
    fn blank_alias_falls_back_to_name() {
        let e = Enrichment { alias: "   ".into(), ..Default::default() };
