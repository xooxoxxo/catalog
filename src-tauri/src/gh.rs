...
pub enum PollResult { Pending, SlowDown, Connected(String), Denied, Expired, Error(String) }

#[derive(Debug, Clone, Serialize, PartialEq)]
pub struct GithubUser { pub login: String, pub avatar_url: String }

#[derive(Debug, Clone, Serialize)]
pub struct GithubStatus { pub connected: bool, pub login: Option<String>, pub avatar_url: Option<String> }
...
pub fn parse_device_code(json: &str) -> Option<DeviceCode> {
    ...
    Some(DeviceCode { device_code: r.device_code, user_code: r.user_code, verification_uri: r.verification_uri, interval: r.interval })
}

#[derive(serde::Deserialize)]
struct PollResp { #[serde(default)] access_token: Option<String>, #[serde(default)] error: Option<String> }
pub fn parse_poll(json: &str) -> PollResult {
    let r: PollResp = match serde_json::from_str(json) { Ok(r) => r, Err(_) => return PollResult::Error("bad response".into()) };
    if let Some(t) = r.access_token {
        if !t.is_empty() { return PollResult::Connected(t); }
    }
...
#[derive(serde::Deserialize)]
struct UserResp { #[serde(default)] login: String, #[serde(default)] avatar_url: String }
pub fn parse_user(json: &str) -> Option<GithubUser> {
    let r: UserResp = serde_json::from_str(json).ok()?;
    if r.login.is_empty() { return None; }
    Some(GithubUser { login: r.login, avatar_url: r.avatar_url })
}
...
pub fn resolve_client_id(c: &str) -> &str {
    if c.trim().is_empty() { DEFAULT_CLIENT_ID } else { c }
}

fn kc_set(token: &str) -> Result<(), String> {
    let ok = Command::new("security")
        .args(["add-generic-password", "-U", "-s", KC_SERVICE, "-a", KC_ACCOUNT, "-w", token])
        .status().map_err(|e| e.to_string())?.success();
    if ok { Ok(()) } else { Err("keychain write failed".into()) }
}
...
pub fn disconnect() -> Result<(), String> {
    let _ = Command::new("security")
        .args(["delete-generic-password", "-s", KC_SERVICE, "-a", KC_ACCOUNT])
        .status();
    Ok(())
}
...
fn post_form(url: &str, body: &str) -> Option<String> {
    let out = Command::new("curl")
        .args(["-s", "--max-time", "20", "-H", "Accept: application/json", "-H", "User-Agent: catalog", "-d", body, url])
        .output().ok()?;
...
fn get_auth(url: &str, token: &str) -> Option<String> {
    let out = Command::new("curl")
        .args(["-s", "--max-time", "20",
            ...
            "-H", "User-Agent: catalog", "-H", "Accept: application/json", url])
        .output().ok()?;
    if out.status.success() { Some(String::from_utf8_lossy(&out.stdout).into_owned()) } else { None }
}
...
pub fn device_start(client_id: &str) -> Result<DeviceCode, String> {
    ...
    parse_device_code(&resp).ok_or_else(|| format!("unexpected response: {}", resp.chars().take(160).collect::<String>()))
}
...
pub fn device_poll(client_id: &str, device_code: &str) -> Result<&'static str, String> {
    ...
    let resp = post_form("https://github.com/login/oauth/access_token", &body).ok_or("request failed")?;
    Ok(match parse_poll(&resp) {
        PollResult::Connected(t) => { kc_set(&t)?; "connected" }
...
pub fn status() -> GithubStatus {
    let disconnected = GithubStatus { connected: false, login: None, avatar_url: None };
    let Some(token) = kc_get() else { return disconnected; };
    match get_auth("https://api.github.com/user", &token).and_then(|j| parse_user(&j)) {
        Some(u) => GithubStatus { connected: true, login: Some(u.login), avatar_url: Some(u.avatar_url) },
        None => disconnected,
    }
}

#[derive(serde::Deserialize)]
struct StarOwner { login: String }
#[derive(serde::Deserialize)]
struct StarRepo {
    full_name: String,
    name: String,
...
fn get_auth_raw(url: &str, token: &str) -> Option<String> {
    ...
    let out = Command::new("curl")
        .args(["-sf", "--max-time", "20",
            "-H", &format!("Authorization: Bearer {token}"),
            "-H", "User-Agent: catalog", "-H", "Accept: application/vnd.github.raw", url])
        .output().ok()?;
    if out.status.success() { Some(String::from_utf8_lossy(&out.stdout).into_owned()) } else { None }
}
...
fn write_auth(method: &str, url: &str, token: &str) -> Result<(), String> {
    let out = Command::new("curl")
        .args(["-s", "-o", "/dev/null", "-w", "%{http_code}", "-X", method, "--max-time", "20",
            "-H", &format!("Authorization: Bearer {token}"),
            "-H", "User-Agent: catalog", "-H", "Content-Length: 0", url])
        .output().map_err(|e| e.to_string())?;
    let code = String::from_utf8_lossy(&out.stdout).trim().to_string();
    if code == "204" || code == "200" { Ok(()) } else { Err(format!("github returned {code}")) }
}
...
pub fn readme(owner: &str, repo: &str) -> Result<String, String> {
    let token = kc_get().ok_or("not connected")?;
    get_auth_raw(&format!("https://api.github.com/repos/{owner}/{repo}/readme"), &token)
        .ok_or_else(|| "no README".into())
}

pub fn star(owner: &str, repo: &str) -> Result<(), String> {
    let token = kc_get().ok_or("not connected")?;
    write_auth("PUT", &format!("https://api.github.com/user/starred/{owner}/{repo}"), &token)
}
pub fn unstar(owner: &str, repo: &str) -> Result<(), String> {
    let token = kc_get().ok_or("not connected")?;
    write_auth("DELETE", &format!("https://api.github.com/user/starred/{owner}/{repo}"), &token)
}
...
mod tests {
    ...
    #[test]
    fn poll_states() {
        assert_eq!(parse_poll(r#"{"access_token":"gho_abc","token_type":"bearer","scope":"read:user"}"#), PollResult::Connected("gho_abc".into()));
        assert_eq!(parse_poll(r#"{"error":"authorization_pending"}"#), PollResult::Pending);
        assert_eq!(parse_poll(r#"{"error":"slow_down","interval":10}"#), PollResult::SlowDown);
        assert_eq!(parse_poll(r#"{"error":"expired_token"}"#), PollResult::Expired);
        assert_eq!(parse_poll(r#"{"error":"access_denied"}"#), PollResult::Denied);
        assert!(matches!(parse_poll(r#"{"error":"unsupported_grant_type"}"#), PollResult::Error(_)));
