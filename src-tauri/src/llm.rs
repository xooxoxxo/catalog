...
pub fn build_prompt(item: &Item) -> String {
    ...
    if let Some(v) = &item.version { ctx.push_str(&format!("\nversion: {v}")); }
    if let Some(p) = &item.exec_path { ctx.push_str(&format!("\npath: {p}")); }
    if let Some(d) = &item.raw_desc { ctx.push_str(&format!("\nknown description: {d}")); }
    if let Some(h) = &item.homepage { ctx.push_str(&format!("\nhomepage: {h}")); }
    format!(
...
pub fn parse_suggestion(stdout: &str) -> Result<Suggestion, String> {
    ...
    if end < start { return Err("malformed JSON span".into()); }
...
pub fn harden_argv(command: &str, argv: Vec<String>) -> Vec<String> {
    ...
    if base == "claude" && !argv.iter().any(|a| a == "--mcp-config") {
        let mut v = vec!["--strict-mcp-config".to_string(), "--mcp-config".to_string(), "{\"mcpServers\":{}}".to_string()];
        v.extend(argv);
        return v;
    }
    argv
...
pub fn run_provider(p: &Provider, prompt: &str) -> Result<String, String> {
    ...
    let out = if p.stdin {
        ...
        cmd.output().map_err(|e| format!("failed to run {bin}: {e}"))?
    };
    if !out.status.success() {
        return Err(format!("{} exited {}: {}", p.command, out.status, String::from_utf8_lossy(&out.stderr)));
    }
...
pub fn check_online() -> bool {
    ...
    "1.1.1.1:443".to_socket_addrs().ok()
...
pub fn describe(item: &Item) -> Result<Suggestion, String> {
    ...
    let p = cfg.providers.get(&cfg.active).ok_or("active provider not found")?;
...
pub fn build_batch_prompt(items: &[Item]) -> String {
    ...
    for it in items {
        blocks.push_str(&format!("\n[{}]\nname: {}\nsource: {}", it.id, it.name, it.source));
        if let Some(v) = &it.version { blocks.push_str(&format!("\nversion: {v}")); }
        if let Some(p) = &it.exec_path { blocks.push_str(&format!("\npath: {p}")); }
        if let Some(d) = &it.raw_desc { blocks.push_str(&format!("\nknown description: {d}")); }
        if let Some(h) = &it.homepage { blocks.push_str(&format!("\nhomepage: {h}")); }
        blocks.push('\n');
    }
    format!(
...
pub fn parse_batch(stdout: &str) -> std::collections::BTreeMap<String, Suggestion> {
    ...
    let (Some(s), Some(e)) = (stdout.find('{'), stdout.rfind('}')) else { return out; };
    if e < s { return out; }
    let val: serde_json::Value = match serde_json::from_str(&stdout[s..=e]) { Ok(v) => v, Err(_) => return out };
    if let Some(obj) = val.as_object() {
        for (k, v) in obj {
            if let Ok(sg) = serde_json::from_value::<Suggestion>(v.clone()) { out.insert(k.clone(), sg); }
        }
    }
    out
}
...
pub fn describe_batch(items: &[Item]) -> Result<std::collections::BTreeMap<String, Suggestion>, String> {
    let cfg = crate::config::load_from(&crate::config::config_path());
    let p = cfg.providers.get(&cfg.active).ok_or("active provider not found")?;
...
pub fn theme_palette(vibe: &str, base: &str) -> Result<std::collections::BTreeMap<String, String>, String> {
    let cfg = crate::config::load_from(&crate::config::config_path());
    let p = cfg.providers.get(&cfg.active).ok_or("active provider not found")?;
...
mod tests {
    ...
    fn item() -> Item {
        Item { id: "orphan:/usr/local/bin/zap".into(), name: "zap".into(), source: "orphan".into(),
            source_detail: None, version: None, exec_path: Some("/usr/local/bin/zap".into()),
            homepage: None, raw_desc: None, installed_on_request: None }
    }
    ...
    fn parses_plain_json() {
        let s = parse_suggestion(r#"{"description":"A fast linter","tags":["cli","dev"]}"#).unwrap();
    ...
    fn theme_prompt_names_all_tokens_and_demands_json() {
        ...
        for tok in ["bg","surface","surface2","line","line2","text","text-2","text-3","accent","accent-fg","update","orphan","danger"] {
    ...
    fn harden_argv_disables_mcp_for_claude_only() {
        ...
        assert_eq!(harden_argv("/opt/homebrew/bin/claude", vec!["-p".into()])[0], "--strict-mcp-config");
        assert_eq!(harden_argv("ollama", vec!["run".into()]), vec!["run".to_string()]); // others untouched
        assert_eq!(harden_argv("claude", vec!["--mcp-config".into(), "x".into()]).len(), 2); // idempotent
    }

    #[test]
    fn run_provider_errors_on_empty_command() {
        let p = Provider { command: String::new(), args: vec![], stdin: false, requires_online: false };
        assert!(run_provider(&p, "x").is_err());
    }

    #[test]
    fn run_provider_stdin_pipes_to_cat_without_deadlock() {
        let p = Provider { command: "cat".into(), args: vec![], stdin: true, requires_online: false };
