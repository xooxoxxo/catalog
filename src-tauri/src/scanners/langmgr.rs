...
struct NpmRoot {
    #[serde(default)]
    dependencies: std::collections::HashMap<String, NpmDep>,
}
#[derive(Deserialize)]
struct NpmDep {
...
pub fn parse_npm_global(json: &str) -> Vec<Item> {
    serde_json::from_str::<NpmRoot>(json)
        .map(|r| {
            r.dependencies
                .into_iter()
                .map(|(n, d)| mk("npm", &n, d.version))
                .collect()
        })
        .unwrap_or_default()
}
...
pub fn parse_pipx_list(json: &str) -> Vec<Item> {
    serde_json::from_str::<PipxRoot>(json)
        .map(|r| {
            r.venvs
                .into_values()
                .map(|v| {
                    mk(
                        "pipx",
                        &v.metadata.main_package.package,
                        v.metadata.main_package.package_version,
                    )
                })
                .collect()
        })
...
pub fn scan() -> Result<Vec<Item>, String> {
    ...
    if let Some(o) = run("npm", &["ls", "-g", "--json"]) {
        items.extend(parse_npm_global(&o));
    }
    if let Some(o) = run("cargo", &["install", "--list"]) {
        items.extend(parse_cargo_install(&o));
    }
    if let Some(o) = run("uv", &["tool", "list"]) {
        items.extend(parse_uv_tool(&o));
    }
    if let Some(o) = run("pipx", &["list", "--json"]) {
        items.extend(parse_pipx_list(&o));
    }
...
mod tests {
    ...
    fn npm_parses() {
        ...
        assert!(v
            .iter()
            .any(|i| i.id == "npm:typescript" && i.version.as_deref() == Some("5.4.5")));
    }
    #[test]
    fn cargo_parses() {
        ...
        assert!(v
            .iter()
            .any(|i| i.id == "cargo:ripgrep" && i.version.as_deref() == Some("14.1.0")));
    ...
    fn pipx_parses() {
        ...
        assert!(v
            .iter()
            .any(|i| i.id == "pipx:poetry" && i.version.as_deref() == Some("1.8.0")));
    }
    #[test]
