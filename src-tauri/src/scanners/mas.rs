    ...
    stdout
        ...
        .filter_map(|line| {
            ...
            let (name, version) = match rest.rsplit_once(" (") {
                Some((n, v)) => (n.trim().to_string(), Some(v.trim_end_matches(')').to_string())),
...
pub fn scan() -> Result<Vec<Item>, String> {
    ...
    let Some(bin) = bin else { return Ok(vec![]); }; // mas not installed → no MAS items
    let out = Command::new(bin).arg("list").output().map_err(|e| e.to_string())?;
