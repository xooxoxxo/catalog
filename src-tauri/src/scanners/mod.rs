...
pub fn merge_mas_with_apps(items: &mut Vec<Item>) {
    ...
    let app_by_name: HashMap<String, String> = items
        .iter()
    ...
    items.retain(|i| {
        !(i.id.starts_with("app:") && i.exec_path.as_ref().is_some_and(|p| adopted.contains(p)))
    });
}
...
mod tests {
    ...
    fn merge_mas_dedups_app_store_apps_keeping_mas_id() {
        ...
        assert_eq!(v.len(), 3);
        assert!(
            !v.iter().any(|i| i.source == "mas"),
            "no mas source should remain"
        );
    }
    ...
    fn scan_all_returns_multiple_sources_with_unique_ids() {
        ...
        assert!(
            items.len() > 50,
            "expected a populated inventory, got {}",
            items.len()
        );
