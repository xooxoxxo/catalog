...
const SYSTEM_DIRS: &[&str] = &["/usr/bin", "/bin", "/usr/sbin", "/sbin", "/usr/libexec", "/System"];
...
pub fn find_orphans(path_execs: &[(String, String)], claimed: &HashSet<String>, manager_prefixes: &[String]) -> Vec<Item> {
    let mut seen = HashSet::new();
    path_execs
        .iter()
        .filter(|(_disp, canon)| !claimed.contains(canon) && !is_system(canon) && !is_manager(canon, manager_prefixes))
        .filter(|(disp, _canon)| !is_manager(disp, manager_prefixes))
        .filter(|(_disp, canon)| seen.insert(canon.clone()))
        .map(|(disp, canon)| {
            let name = Path::new(disp).file_name().map(|s| s.to_string_lossy().into_owned()).unwrap_or_else(|| disp.clone());
            Item {
...
pub fn scan(claimed: &HashSet<String>) -> Vec<Item> {
    ...
    for dir in path_var.split(':').filter(|d| !d.is_empty() && !is_system(d)) {
        let Ok(rd) = std::fs::read_dir(dir) else { continue; };
        for entry in rd.filter_map(|e| e.ok()) {
            let p = entry.path();
            if is_executable(&p) {
                let disp = p.to_string_lossy().into_owned();
                let canon = std::fs::canonicalize(&p).map(|c| c.to_string_lossy().into_owned()).unwrap_or_else(|_| disp.clone());
...
mod tests {
    use super::*;
    fn claimed(paths: &[&str]) -> HashSet<String> { paths.iter().map(|s| s.to_string()).collect() }
    ...
    fn claimed_is_not_orphan() {
        let execs = vec![("/opt/homebrew/bin/eza".into(), "/opt/homebrew/Cellar/eza/1/bin/eza".into())];
        let v = find_orphans(&execs, &claimed(&["/opt/homebrew/Cellar/eza/1/bin/eza"]), &[]);
        assert_eq!(v.len(), 0);
    }
    #[test]
    ...
    fn brew_bin_is_not_orphan_even_when_unclaimed() {
        let execs = vec![("/opt/homebrew/bin/eza".to_string(), "/opt/homebrew/Cellar/eza/1/bin/eza".to_string())];
