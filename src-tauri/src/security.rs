...
struct BatchResp { #[serde(default)] results: Vec<BatchResult> }
#[derive(serde::Deserialize)]
struct BatchResult { #[serde(default)] vulns: Vec<BatchVuln> }
#[derive(serde::Deserialize)]
struct BatchVuln { id: String }
...
pub fn parse_querybatch(json: &str) -> Vec<Vec<String>> {
    match serde_json::from_str::<BatchResp>(json) {
        Ok(r) => r.results.into_iter().map(|res| res.vulns.into_iter().map(|v| v.id).collect()).collect(),
        Err(_) => vec![],
    }
}
...
struct VulnResp {
    ...
    #[serde(default)] affected: Vec<Affected>,
    #[serde(default)] database_specific: DbSpecific,
}
#[derive(serde::Deserialize, Default)]
struct DbSpecific { #[serde(default)] severity: Option<String> }
#[derive(serde::Deserialize)]
struct Affected { #[serde(default)] ranges: Vec<VRange> }
#[derive(serde::Deserialize)]
struct VRange { #[serde(default)] events: Vec<VEvent> }
#[derive(serde::Deserialize)]
struct VEvent { #[serde(default)] fixed: Option<String> }

pub fn severity_of(id: &str, db_sev: Option<&str>) -> String {
    if id.starts_with("MAL-") { return "MALWARE".to_string(); }
...
pub fn parse_vuln(json: &str) -> Option<VulnInfo> {
    ...
    Some(VulnInfo { id: v.id, aliases: v.aliases, summary, severity, fixed, url })
}

fn curl_post_json(url: &str, body: &str) -> Option<String> {
    let mut child = Command::new("curl")
        .args(["-s", "--max-time", "20", "-H", "Content-Type: application/json", "--data-binary", "@-", url])
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .spawn()
        .ok()?;
...
fn curl_get(url: &str) -> Option<String> {
    let out = Command::new("curl").args(["-s", "--max-time", "20", url]).output().ok()?;
    if out.status.success() { Some(String::from_utf8_lossy(&out.stdout).into_owned()) } else { None }
}
...
pub fn scan<F: Fn(usize, usize)>(items: &[Item], progress: F) -> Vec<SecurityFinding> {
    let q: Vec<(&Item, (String, String, String))> =
        items.iter().filter_map(|it| query_for(it).map(|t| (it, t))).collect();
    if q.is_empty() { return vec![]; }

    let queries: Vec<String> = q.iter().map(|(_, (eco, name, ver))| {
        format!(r#"{{"package":{{"ecosystem":"{}","name":"{}"}},"version":"{}"}}"#,
            eco, json_escape(name), json_escape(ver))
    }).collect();
    ...
    let affected: Vec<usize> = ids_per.iter().enumerate()
    ...
    for (done, &i) in affected.iter().enumerate() {
        ...
        for id in &ids_per[i] {
            if let Some(d) = curl_get(&format!("https://api.osv.dev/v1/vulns/{id}")).and_then(|j| parse_vuln(&j)) {
                vulns.push(d);
            }
        }
        if !vulns.is_empty() {
...
mod tests {
    ...
    fn query_for_requires_ecosystem_and_version() {
        assert_eq!(query_for(&item("npm", "lodash", Some("4.17.4"))),
    ...
    fn querybatch_parses_parallel_results() {
        ...
        assert_eq!(parse_querybatch(json), vec![vec!["GHSA-1".to_string(), "CVE-2".to_string()], vec![] as Vec<String>, vec!["MAL-3".to_string()]]);
