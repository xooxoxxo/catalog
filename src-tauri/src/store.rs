...
pub fn rebuild(conn: &Connection, items: &[Item]) -> Result<usize, rusqlite::Error> {
    ...
    for it in items {
        stmt.execute(params![
            it.id, it.name, it.source, it.source_detail, it.version,
            it.exec_path, it.homepage, it.raw_desc,
...
pub fn query_all(conn: &Connection) -> Result<Vec<Item>, rusqlite::Error> {
    ...
    let rows = stmt.query_map([], |r| {
        Ok(Item {
            id: r.get(0)?, name: r.get(1)?, source: r.get(2)?,
            source_detail: r.get(3)?, version: r.get(4)?, exec_path: r.get(5)?,
            homepage: r.get(6)?, raw_desc: r.get(7)?,
...
pub fn get(conn: &Connection, id: &str) -> Result<Option<Item>, rusqlite::Error> {
    ...
    let mut rows = stmt.query_map([id], |r| {
        Ok(Item {
            id: r.get(0)?, name: r.get(1)?, source: r.get(2)?, source_detail: r.get(3)?,
            version: r.get(4)?, exec_path: r.get(5)?, homepage: r.get(6)?, raw_desc: r.get(7)?,
            installed_on_request: r.get::<_, Option<i64>>(8)?.map(|i| i != 0),
        })
    })?;
    rows.next().transpose()
...
mod tests {
    ...
    fn sample() -> Item {
        Item { id: "brew:eza".into(), name: "eza".into(), source: "brew".into(),
            source_detail: None, version: Some("0.23.4".into()), exec_path: None,
            homepage: None, raw_desc: Some("modern ls".into()), installed_on_request: Some(true) }
    }
