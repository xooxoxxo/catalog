...
pub fn get_man(name: &str) -> Result<String, String> {
    ...
    if text.trim().is_empty() { Err(format!("no manual entry for {name}")) } else { Ok(text) }
}

/// Reveal a file/app in Finder.
pub fn reveal_in_finder(path: &str) -> Result<(), String> {
    let st = Command::new("open").arg("-R").arg(path).status().map_err(|e| e.to_string())?;
    if st.success() { Ok(()) } else { Err(format!("open -R exited {st}")) }
}
...
pub fn copy_text(text: &str) -> Result<(), String> {
    ...
    child.stdin.as_mut().ok_or("no pbcopy stdin")?.write_all(text.as_bytes()).map_err(|e| e.to_string())?;
    let st = child.wait().map_err(|e| e.to_string())?;
    if st.success() { Ok(()) } else { Err("pbcopy failed".into()) }
}
