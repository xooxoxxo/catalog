...
pub struct Fonts { pub ui: String, pub mono: String }
...
fn themes_path() -> PathBuf {
    crate::config::config_base().join("catalog").join("themes.json")
}
...
pub fn list() -> Vec<Theme> { load_from(&themes_path()) }
pub fn save(t: Theme) -> Result<(), String> { write_to(&themes_path(), &upsert(list(), t)) }
...
mod tests {
    ...
    fn theme(id: &str) -> Theme {
        Theme { id: id.into(), name: id.into(), base: "dark".into(),
            colors: BTreeMap::new(), fonts: Fonts { ui: "u".into(), mono: "m".into() }, radius: 10.0 }
    }
    #[test]
    fn upsert_replaces_or_appends() {
        let v = upsert(vec![theme("a")], theme("b"));
        assert_eq!(v.len(), 2);
        let mut t = theme("a"); t.name = "renamed".into();
        let v2 = upsert(v, t);
        assert_eq!(v2.len(), 2);
